...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void initTable(SQLiteDatabase db) {
        ...
        String sql = "create table user_table(user_id integer primary key autoincrement, username text, password text, phone text unique, email_address text, signature text, is_auth boolean, pic text)";   //pic_id varchar(120)
        db.execSQL(sql);
    ...
    private void initData(SQLiteDatabase db) {
        ...
        String sql = "insert into user_table(username, password, phone, email_address, signature, is_auth, pic) values('admin', 'admin', null, '1078957527@qq.com', null, 1, 'drawable://"+R.drawable.ic_appbar_user+"')";
        db.execSQL(sql);
        sql = "insert into user_table(username, password, phone, email_address, signature, is_auth) values('0', '0', null, null, '一段个性签名', 0)";
        db.execSQL(sql);
        sql = "insert into user_table(username, password, phone, email_address, signature, is_auth, pic) values('1', '1', null, null, '一段个性签名', 0, 'drawable://"+R.drawable.ic_appbar_user+"')";
        db.execSQL(sql);
